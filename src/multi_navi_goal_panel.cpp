...
namespace navi_multi_goals_pub_rviz_plugin {
            ...
            : rviz_common::Panel(parent), maxNumGoal_(1) 
    {
        ...
        status_sub_ = nh_->create_subscription<std_msgs::msg::Empty>("/mission_complete", 1,
...
//        pose_array_.poses.resize(maxNumGoal_);
    ...
    void MultiNaviGoalsPanel::startNavi() {
        ...
        if (!pose_array_.poses.empty() && curGoalIdx_ < maxNumGoal_) {
            pose_array_.header.stamp = nh_->get_clock()->now();
    ...
    void MultiNaviGoalsPanel::statusCB(const std_msgs::msg::Empty::SharedPtr ) {
        if (rclcpp::ok() && permit_ && cycle_) {
            startNavi();
        }
    }
